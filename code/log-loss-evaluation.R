...
logloss.evaluate <- function(predictions_file_loc ="", predictions_vector = NULL){
  if(length(predictions_vector) > 0){
    ## load actual results
    ...
    X.res$id <- ifelse(X.res$wteamid < X.res$lteamid, paste(X.res$season, 
        X.res$wteamid, X.res$lteamid, sep = '_'), paste(X.res$season, X.res$lteamid, X.res$wteamid, sep = '_'))
    
    ## Merge actual results with predictions
    X <- merge(predictions_vector, X.res[,c('id', 'wteamid', 'lteamid')])
    ## compute log-loss
    # compute y
